...
def init():
    ...
    global all_keys
    all_keys = list(_KEY_ABILITIES)
    global all_candles
    all_candles = list(_CANDLE_TO_ABILITY)
    global all_abilities
    all_abilities = list(_CANDLE_TO_ABILITY.values())
    #Logic
    ...
    while True:
        ...
        if check_to_requirement:
            ...
            for requirement in chosen_requirement_list:
                for num in range(_KEY_ABILITIES[requirement]):
...
def move_through_rooms():
    ...
    while True:
        for door in copy.deepcopy(current_available_doors):
            ...
            if room in Data.constant["BloodlessRoomRequirement"]:
                for check, requirement in Data.constant["BloodlessRoomRequirement"][room][door].items():
        ...
        for special_check in special_check_to_requirement:
            if special_check in special_check_to_door and special_check_to_requirement[special_check]():
                for door in special_check_to_door[special_check]:
                    analyse_check(special_check, Data.constant["BloodlessRoomRequirement"][Item.get_door_room(door)][door][special_check])
...
def get_check_type(check):
    ...
    if check in Data.constant["EnemyInfo"] or check == "N2008_BOSS":
...
def den_portal_available():
    for boss in _BOSS_REQUIREMENTS:
...
def update_shard_candles():
    ...
    for item in ability_to_location:
        ...
        old_value = f"EPBBloodlessAbilityType::{_CANDLE_TO_ABILITY[ability_to_location[item]]}"
        new_value = f"EPBBloodlessAbilityType::{item}"
        Data.search_and_replace_string(file_name, class_name, data_name, old_value, new_value)

def increase_starting_stats():
    #Give Bloodless 4 of each stat to start with
    for stat in ["STR", "INT", "CON", "MND"]:
        for index in range(4):
            Data.datatable["PB_DT_BloodlessAbilityData"][f"BLD_ABILITY_{stat}_UP_{index + 1}"]["IsUnlockedByDefault"] = True
...
def remove_gremory_cutscene():
    ...
    Data.datatable["PB_DT_EventFlagMaster"]["Event_15_001_0000"]["Id"]          = Data.datatable["PB_DT_EventFlagMaster"]["Event_24_001_0000"]["Id"]
    Data.datatable["PB_DT_EventFlagMaster"]["Event_15_geki_End"]["Id"]          = Data.datatable["PB_DT_EventFlagMaster"]["Event_24_001_0000"]["Id"]
    Data.datatable["PB_DT_EventFlagMaster"]["Event_15_Cine_End"]["Id"]          = Data.datatable["PB_DT_EventFlagMaster"]["Event_24_001_0000"]["Id"]
    Data.datatable["PB_DT_EventFlagMaster"]["Event_15_N1008_BattleStart"]["Id"] = Data.datatable["PB_DT_EventFlagMaster"]["Event_24_001_0000"]["Id"]
    Data.datatable["PB_DT_RoomMaster"]["m18ICE_018"]["BgmID"] = "BGM_BOSS_B"
...
def create_log(seed, map):
    ...
    log["Key"]  = {}
    for item in ability_to_location:
        item_name = Data.translation["Bloodless"][Utility.remove_inst_number(item)]
