...
def init():
    ...
    enemy_replacement : dict[str, str] = {}
    global enemy_replacement_invert
    enemy_replacement_invert : dict[str, str] = {}
    global available_portrait_index
    available_portrait_index = []


def set_enemy_level_weight(weight : int):
    global enemy_level_weight
    enemy_level_weight = WEIGHT_EXPONENTS[weight - 1]

def set_boss_level_weight(weight : int):
    global boss_level_weight
    boss_level_weight = WEIGHT_EXPONENTS[weight - 1]

def set_enemy_tolerance_weight(weight : int):
    global enemy_tolerance_weight
    enemy_tolerance_weight = WEIGHT_EXPONENTS[weight - 1]

def set_boss_tolerance_weight(weight : int):
    global boss_tolerance_weight
    boss_tolerance_weight = WEIGHT_EXPONENTS[weight - 1]
...
def get_original_enemy_stats():
    ...
    for entry in Data.datatable["PB_DT_CharacterParameterMaster"]:
        ...
        original_enemy_stats[entry]["Level"] = Data.datatable["PB_DT_CharacterParameterMaster"][entry]["DefaultEnemyLevel"]
        original_enemy_stats[entry]["POI"]   = Data.datatable["PB_DT_CharacterParameterMaster"][entry]["POI"]
        original_enemy_stats[entry]["CUR"]   = Data.datatable["PB_DT_CharacterParameterMaster"][entry]["CUR"]
        original_enemy_stats[entry]["STO"]   = Data.datatable["PB_DT_CharacterParameterMaster"][entry]["STO"]
        original_enemy_stats[entry]["SLO"]   = Data.datatable["PB_DT_CharacterParameterMaster"][entry]["SLO"]
...
def convert_area_to_progress():
    ...
    for suffix in ["", "Original", "BloodlessMode", "BloodlessModeOriginal"]:
        ...
        for num in range(len(Data.constant[entry])):
            area_to_progress[entry][Data.constant[entry][num]] = num + 1.0
...
def set_zangetsu_enemy_exp():
    ...
    for entry in Data.datatable["PB_DT_CharacterParameterMaster"]:
        ...
        if entry in zangetsu_exp:
            Data.datatable["PB_DT_CharacterParameterMaster"][entry]["Experience99Enemy"] = zangetsu_exp[entry]
            Data.datatable["PB_DT_CharacterParameterMaster"][entry]["Experience"]        = zangetsu_exp[entry]
        elif entry[0:5] in zangetsu_exp:
            Data.datatable["PB_DT_CharacterParameterMaster"][entry]["Experience99Enemy"] = zangetsu_exp[entry[0:5]]
            Data.datatable["PB_DT_CharacterParameterMaster"][entry]["Experience"]        = zangetsu_exp[entry[0:5]]
        else:
...
def rebalance_enemies_to_map():
    ...
    for enemy in Data.datatable["PB_DT_CharacterParameterMaster"]:
        if not is_enemy(enemy) or enemy in _STATIC_ENEMIES or is_final_boss(enemy):
            continue
...
def randomize_level_for(entry, weight):
    for suffix in ["", "BloodlessMode"]:
        ...
        default_level = 50 if is_final_boss(entry) else Data.datatable["PB_DT_CharacterParameterMaster"][entry][suffix + "DefaultEnemyLevel"]
        #Patch level
...
def randomize_tolerances_for(entry, weight):
    ...
    if Data.datatable["PB_DT_CharacterParameterMaster"][entry]["ZAN"] == 100.0 and Data.datatable["PB_DT_CharacterParameterMaster"][entry]["DAG"] == 100.0 and Data.datatable["PB_DT_CharacterParameterMaster"][entry]["TOT"] == 100.0 and Data.datatable["PB_DT_CharacterParameterMaster"][entry]["FLA"] == 100.0 and Data.datatable["PB_DT_CharacterParameterMaster"][entry]["ICE"] == 100.0 and Data.datatable["PB_DT_CharacterParameterMaster"][entry]["LIG"] == 100.0 and Data.datatable["PB_DT_CharacterParameterMaster"][entry]["HOL"] == 100.0 and Data.datatable["PB_DT_CharacterParameterMaster"][entry]["DAR"] == 100.0:
        return
    #Weak points
        ...
        for door in Data.constant["RoomRequirement"][room]:
            for check in list(Data.constant["RoomRequirement"][room][door]):
                ...
                if enemy_profile[0] in enemy_replacement:
                    ...
                    if not (Data.datatable["PB_DT_RoomMaster"][room]["AreaHeightSize"] < 2 and enemy_replacement[enemy_profile[0]] in ["N3082", "N3121"]):
                        Data.constant["RoomRequirement"][room][door][enemy_replacement[enemy_profile[0]] + suffix] = Data.constant["RoomRequirement"][room][door][check]
                    del Data.constant["RoomRequirement"][room][door][check]
    #Remove the Gusions from library and labs if they were scaled up
    if enemy_replacement_invert["N3035"] in _LARGES_ENEMIES:
...
def restore_enemy_scaling():
    ...
    for entry in Data.datatable["PB_DT_CharacterParameterMaster"]:
        ...
        if enemy_id in enemy_replacement:
            ...
            if enemy_id in _LARGES_ENEMIES and not enemy_replacement_invert[enemy_id] in _LARGES_ENEMIES:
            ...
            if not enemy_id in _LARGES_ENEMIES and enemy_replacement_invert[enemy_id] in _LARGES_ENEMIES:
                Data.datatable["PB_DT_CharacterParameterMaster"][entry]["CapsuleRadius"]     *= 1.5
                Data.datatable["PB_DT_CharacterParameterMaster"][entry]["CapsuleHeight"]     *= 1.5
                Data.datatable["PB_DT_CharacterParameterMaster"][entry]["MeshScaleX"]        *= 1.5
                Data.datatable["PB_DT_CharacterParameterMaster"][entry]["MeshScaleY"]        *= 1.5
    ...
    for entry in Data.datatable["PB_DT_ArchiveEnemyMaster"]:
        ...
        if enemy_id in enemy_replacement:
            if enemy_replacement_invert[enemy_id] in enemy_id_to_archive:
                Data.datatable["PB_DT_ArchiveEnemyMaster"][entry]["Area1"] = Data.original_datatable["PB_DT_ArchiveEnemyMaster"][enemy_id_to_archive[enemy_replacement_invert[enemy_id]]]["Area1"]
                Data.datatable["PB_DT_ArchiveEnemyMaster"][entry]["Area2"] = Data.original_datatable["PB_DT_ArchiveEnemyMaster"][enemy_id_to_archive[enemy_replacement_invert[enemy_id]]]["Area2"]
                Data.datatable["PB_DT_ArchiveEnemyMaster"][entry]["Area3"] = Data.original_datatable["PB_DT_ArchiveEnemyMaster"][enemy_id_to_archive[enemy_replacement_invert[enemy_id]]]["Area3"]
                Data.datatable["PB_DT_ArchiveEnemyMaster"][entry]["Area4"] = Data.original_datatable["PB_DT_ArchiveEnemyMaster"][enemy_id_to_archive[enemy_replacement_invert[enemy_id]]]["Area4"]
            else:
...
def scale_status_resistances(entry):
    for attr in _STATUS_RESISTANCES:
        ...
        if original_enemy_stats[entry]["POI"] == 100.0 and original_enemy_stats[entry]["CUR"] == 100.0 and original_enemy_stats[entry]["STO"] >= 99.0 and original_enemy_stats[entry]["SLO"] == 100.0:
            continue
        #Gain
        if Data.datatable["PB_DT_CharacterParameterMaster"][entry]["DefaultEnemyLevel"] > original_enemy_stats[entry]["Level"] + ((99 - original_enemy_stats[entry]["Level"]) * 0.10):
            attr_num += 25.0
        if Data.datatable["PB_DT_CharacterParameterMaster"][entry]["DefaultEnemyLevel"] > original_enemy_stats[entry]["Level"] + ((99 - original_enemy_stats[entry]["Level"]) * 0.55):
            attr_num += 25.0
        attr_num = min(attr_num, 99.99 if attr == "STO" else 100.0)
        #Loss
        if Data.datatable["PB_DT_CharacterParameterMaster"][entry]["DefaultEnemyLevel"] < original_enemy_stats[entry]["Level"] * 0.90:
            attr_num = math.ceil(attr_num - 25.0)
        if Data.datatable["PB_DT_CharacterParameterMaster"][entry]["DefaultEnemyLevel"] < original_enemy_stats[entry]["Level"] * 0.45:
            attr_num -= 25.0
...
def update_special_properties():
    ...
    for entry in ["N1001", "N1001_HEAD"]:
        Data.datatable["PB_DT_CharacterParameterMaster"][entry]["BloodlessModeEnemyHPOverride"] = int(calculate_stat_with_level(entry, Data.datatable["PB_DT_CharacterParameterMaster"][entry]["BloodlessModeDefaultEnemyLevel"], "MaxHP")*2.0)
    #Update Gebel's health threshold for the red moon based on his level
    Data.datatable["PB_DT_CoordinateParameter"]["FakeMoon_ChangeHPThreshold"]["Value"] = int(calculate_stat_with_level("N1012", Data.datatable["PB_DT_CharacterParameterMaster"]["N1012"]["DefaultEnemyLevel"], "MaxHP")*0.15)
...
def create_log():
    ...
    for enemy in Data.constant["EnemyInfo"]:
        ...
        if enemy in enemy_replacement:
            log[enemy_name]["Position"] = Data.translation["Enemy"][enemy_replacement_invert[enemy]] if enemy_replacement[enemy] != enemy else "Unchanged"
        else:
