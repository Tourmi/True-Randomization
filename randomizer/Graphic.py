...
_SHARD_TYPE_TO_HSV : dict[str, HSV] = {
    "Skill":       (  0,   0, 100),
    "Trigger":     (  0, 100, 100),
...
class Graphic:
    @staticmethod
    def update_default_outfit_hsv(parameter_string : str):
        ...
        for index in range(6):
            for parameter in parameter_list:
                Data.datatable["PB_DT_HairSalonOldDefaults"]["Body_01_" + "{:02d}".format(index + 1)][parameter[0] + "1"] = int(parameter[1:4])

    @classmethod
    def randomize_backer_portraits(cls):
        #Shuffle backer portraits in a dict
        portraits : list[str] = []
        for directory in os.listdir(ASSETS_DIR + "\\" + Data.file_to_path["Ml_N3100_picture_001"]):
            file_name = os.path.splitext(directory)[0]
            portraits.append(file_name)
        
        new_list = copy.deepcopy(portraits)
        random.shuffle(new_list)
        cls.portrait_replacement = dict(zip(portraits, new_list))

    @classmethod
    def update_backer_portraits(cls):
        #Update the portrait material pointer
        for portrait in cls.portrait_replacement:
            cls.update_portrait_pointer(portrait, cls.portrait_replacement[portrait])

    @classmethod
    def update_portrait_pointer(cls, portrait : str, portrait_replacement : str):
        #Simply swap the file's name in the name map and save as the new name
        portrait_replacement_data = UAsset(f"{ASSETS_DIR}\\{Data.file_to_path[portrait_replacement]}\\{portrait_replacement}.uasset", EngineVersion.VER_UE4_22)
        ...
        portrait_replacement_data.Write(f"{MOD_DIR}\\{Data.file_to_path[portrait]}\\{portrait}.uasset")
    ...
    def update_boss_crystal_color():
        ...
        for file in Data.file_to_path:
            if Data.file_to_type[file] == FileType.Material:
                enemy_id = Data.file_to_path[file].split("\\")[-2]
                if Enemy.is_boss(enemy_id) or enemy_id == "N2008":
                    shard_name = Data.datatable["PB_DT_DropRateMaster"][f"{enemy_id}_Shard"]["ShardId"]
                    shard_type = Data.datatable["PB_DT_ShardMaster"][shard_name]["ShardType"]
                    shard_hsv  = _SHARD_TYPE_TO_HSV[shard_type.split("::")[-1]]
                    Graphic.set_material_hsv(file, "ShardColor", shard_hsv)

    @staticmethod
    def set_material_hsv(filename : str, parameter : str, new_hsv : HSV):
        #Change a vector color in a material file
        #Here we use hsv as a base as it is easier to work with
        if Data.file_to_type[filename] != FileType.Material:
        ...
        if filename in _MATERIAL_TO_OFFSET:
            for offset in _MATERIAL_TO_OFFSET[filename]:
                #Check if given offset is valid
                string = ""
                for num in range(12):
                    string += "{:02x}".format(Data.game_data[filename].Exports[0].Extras[offset + num]).upper()
                ...
                for num in range(3):
                    ...
                    for index in range(4):
                        list.insert(0, "{:02x}".format(Data.game_data[filename].Exports[0].Extras[offset + 12 + num*4 + index]).upper())
                    string = ""
                    for index in list:
                        string += index
                ...
                new_hue = hue if new_hsv[0] < 0 else new_hsv[0]/360
                new_sat = sat if new_hsv[1] < 0 else new_hsv[1]/100
                new_val = val if new_hsv[2] < 0 else new_hsv[2]/100
                ...
                for num in range(3):
                    ...
                    for index in range(4):
                        Data.game_data[filename].Exports[0].Extras[offset + 12 + num*4 + index] = int(list[index], 16)
        #Otherwise change color through the exports
        else:
            for data in Data.game_data[filename].Exports[0].Data:
    ...
    def import_mesh(filename : str):
        ...
        if not os.path.isdir(f"{MOD_DIR}\\{filepath}"):
            os.makedirs(f"{MOD_DIR}\\{filepath}")
        new_file.Write(f"{MOD_DIR}\\{filepath}\\{filename}.uasset")

    @staticmethod
    def import_texture(filename : str):
        #Convert DDS to game assets dynamically instead of cooking them within Unreal Editor
        absolute_asset_dir   = os.path.abspath(f"{ASSETS_DIR}\\{Data.file_to_path[filename]}")
        absolute_texture_dir = os.path.abspath("Data\\Texture")
        absolute_mod_dir     = os.path.abspath(f"{MOD_DIR}\\{Data.file_to_path[filename]}")
